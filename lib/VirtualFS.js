...
import { realloc, concat, copy } from './bufferUtils.js';

type path = string | ArrayBuffer | {pathname: string};
...
class VirtualFS {
  ...
  mmapSync (length: number, flags: number, fdIndex: number, offset: number = 0): ArrayBuffer {
  ...
  readFileSync (file: file, options?: options): string|ArrayBuffer {
    options = this._getOptions({encoding: null, flag: 'r'}, options);
    let fdIndex;
    try {
      const buffer = new ArrayBuffer(4096);
      let totalBuffer = new ArrayBuffer(0);
      let bytesRead = null;
      if (typeof file === 'number') {
        while (bytesRead !== 0) {
