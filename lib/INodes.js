...
import { concat, realloc, copy } from "./bufferUtils.js";
...
class File extends INode {
  ...
  write (buffer: ArrayBuffer, position: number, append: boolean) {
    let data = this._data;
    let bytesWritten;
    if (append) {
...
class Fifo extends INode {
  ...
  pollQueue: (number) => void;
  readQueue: (ArrayBuffer) => void;
  mode: number;
  ...
  setMode(request: number, state: boolean) {
    if (state)
      this.mode |= (1 << request);
    else
      this.mode &= ~(1 << request)
  ...
  notify(size: number) {
    for (let poll = this.pollQueue.shift(); poll !== undefined; poll = this.pollQueue.shift())
      poll(size);
  }

  sendEof() {
    for (let read = this.readQueue.shift(); read !== undefined; read = this.readQueue.shift())
  ...
  read() {
    ...
    if (buf === undefined) {
      if (this.writer === 2) // writer has already closed the fifo
  ...
  addPollSub() {
    if (this.messages.length !== 0)
      return Promise.resolve(this.messages[0].byteLength);
...
export { File, Directory, Symlink, CharacterDev, INodeManager, Fifo };
